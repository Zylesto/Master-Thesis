...
To enhance the functionality of \ac{VA}, it is essential to conduct a thorough review of existing research in vulnerability classification. This chapter provides an overview of current models, tools, and approaches in the field. The insights gained will serve as a foundation for the development of a multidimensional classification model and algorithms to assess and prioritize vulnerabilities in software projects.
...
Other models, such as \ac{CWE}\footnote{\url{https://cwe.mitre.org/}} and \ac{CAPEC}\footnote{\url{https://capec.mitre.org/}}, primarily focus on categorizing weaknesses and attack patterns. While these models are valuable in broader security contexts, they are less applicable to the specific goals of \ac{VA}, which focuses on vulnerability classification and remediation.

By integrating \ac{CVE} as the backbone for vulnerability identification and leveraging complementary classification models such as \ac{CVSS}, \ac{EPSS}, and \ac{SSVC}, \ac{VA} ensures a comprehensive approach to assessing and addressing security vulnerabilities.

\subsection{Common Vulnerabilities and
Exposure}
\label{subsec:common-vulnerabilities-exposure}

The \ac{CVE} system provides a standardized method for identifying and referencing publicly disclosed cybersecurity vulnerabilities and exposures. It is maintained by the \ac{MITRE} and funded by the \ac{CISA}. Each vulnerability in the \ac{CVE} system is assigned a unique identifier, such as \texttt{CVE-2024-12345}, which acts as a universal reference point for tools, databases, and discussions related to cybersecurity.
...
While \ac{CVE} is not a classification or scoring system, it plays a foundational role in vulnerability management by serving as the baseline for tools like the \ac{CVSS}, which assess the severity of vulnerabilities, and the \ac{EPSS}, which predicts their likelihood of exploitation. The \ac{CVE} entries contain basic metadata, such as the vulnerability description, affected products, and references to further details. However, \ac{CVE} itself does not provide technical details, exploit code, or remediation instructions, instead linking to external sources for additional information.
...
The \ac{CVSS} has evolved over time to address the limitations of earlier versions. Each new version introduces improvements to enhance the accuracy and usability of vulnerability assessments.
...
Overall, \ac{EPSS} significantly advances security risk assessment by offering an open, data-driven method that directly predicts the likelihood of a vulnerability being exploited. Unlike \ac{CVSS}, which rates severity based on inherent characteristics without considering actual exploitation probability, \ac{EPSS} uses empirical data and machine learning to focus on vulnerabilities posing the greatest real-world threat, enhancing decision-making in IT security \autocite{jacobs_exploit_2021}.
...
In terms of remediation, InsightVM integrates automated workflows to facilitate the resolution of vulnerabilities. Once vulnerabilities are assessed and prioritized, they can be assigned directly to remediation teams via ticketing systems such as Jira \footnote{\url{https://www.atlassian.com/de/software/jira}} and ServiceNow \footnote{\url{https://www.servicenow.com/}} \autocite{rapid7_live_2017}.

\subsection{Snyk}
\label{subsec:snyk}

Snyk is a platform for vulnerability detection and remediation that employs a multidimensional approach. In addition to traditional vulnerability detection, Snyk monitors multiple vulnerability databases, such as \ac{CVE}s from the \ac{NVD} and others, to ensure comprehensive coverage of known vulnerabilities. Snyk also tracks user activity on GitHub, including issues, pull requests (PRs), and commit messages that may indicate the presence of a security flaw. This is complemented by bulk research, using tools to identify repeated security patterns across open-source packages, and manual audits conducted by the Snyk Security team to scrutinize widely-used packages for vulnerabilities \autocite{snyk_snyk_2024}. 
...
\begin{itemize}
    ...
    \item \textbf{References with Categorical Labels}: Links to sources such as the MITRE CVE List and \ac{NVD}, which categorize the content of each reference.
    \item \textbf{Normalized Multiword Expressions}: Extracted from the vulnerability descriptions in the MITRE CVE List to enhance semantic analysis.
...
Another aspect of EPSS is the collection of timing information. It is not sufficient to know that a Metasploit module exists for a particular \ac{CVE}; the database tracks when the module was added. This enables the correlation of exploitation activity in the wild before and after the module's introduction. This attention to timing applies to all data sources, ensuring that the evolving threat landscape is  reflected in \ac{EPSS} predictions \autocite{first_epss_2024}.
...
\begin{figure}[H]
    ...
    \caption{Distribution of EPSS probabilities as of 2022-03-04. This histogram illustrates the likelihood of exploitation for different \ac{CVE}s, showing that the majority of vulnerabilities have a low exploitation probability \autocite{first_exploit_2022}.}
...
While databases such as the \ac{DST} \footnote{\url{https://security-tracker.debian.org/tracker/}} and the \ac{RHD} \footnote{\url{https://access.redhat.com/security/security-updates/cve}} also offer valuable insights for vulnerability management, they are not examined in detail in this study due to their limited scope and specific focus on particular software ecosystems. These databases primarily target vulnerabilities within their respective platforms — Debian-based and Red Hat systems — which can limit their general applicability across a wider range of software environments. Additionally, the primary focus of this work is on vulnerability databases that provide broader, cross-platform coverage and have higher adoption rates across diverse development ecosystems. As such, databases like the \ac{NVD}, \ac{OSV}, and \ac{GAD} were selected for their comprehensive, ecosystem-agnostic data, which is critical for a generalized approach to vulnerability classification and remediation across open-source and proprietary software landscapes.

\section{Limitations in Vulnerability Classification and Remediation}
\label{sec:limitations-vulnerability}

Overall, the analyzed vulnerability classification models and tools demonstrate a wide range of approaches for identifying, assessing, and prioritizing security vulnerabilities. Frameworks such as \ac{CVSS} and \ac{EPSS} offer valuable structural and data-driven evaluations, while specialized models like \ac{SSVC} provide context-specific and stakeholder-focused perspectives. The examined vulnerability databases, including \ac{NVD}, \ac{OSV}, and \ac{GAD}, supply essential information foundations crucial for comprehensive risk analysis. However relying solely on the unidimensional \ac{CVSS}, as currently implemented in \ac{VA}, can lead to inaccurate prioritizations and impede effective vulnerability remediation. Multidimensional models, such as those employed by \ac{R7IVM} and \ac{TVM}, utilize multiple factors — including exploitability, threat intelligence, asset criticality, and contextual business impacts — to complement the \ac{CVSS}. By integrating additional dimensions, these models enhance the accuracy and relevance of vulnerability prioritization, ensuring that resources are allocated to the most critical and exploitable vulnerabilities. These insights underscore the necessity for a multidimensional classification model that amalgamates the strengths of current systems while mitigating their weaknesses. By developing such a model, the functionality of \ac{VA} can be substantially enhanced, enabling a more precise and holistic evaluation and prioritization of vulnerabilities in software projects.