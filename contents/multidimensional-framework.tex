...
\begin{itemize}
    \item \ac{CVE} ID, \ac{CVSS} Score, and \ac{EPSS} Score:
    Sourced from databases such as the \ac{NVD}, \ac{OSV}, FIRST.org, and \ac{GAD}.
    The \ac{CVE} identifier provides a standardized reference for the vulnerability, while \ac{CVSS} and \ac{EPSS} offer insights into technical severity and exploitation likelihood, respectively.
\end{itemize}
...
The \ac{SSVC} framework, as discussed in section~\ref{subsec:stakeholder-specific-vulnerability}, does not directly affect the vulnerability score but guides remediation by aligning recommendations with organizational priorities. As outlined in section~\ref{subsec:summary_impact}, it mirrors industry solutions (e.g., Tenable, Rapid7, Snyk) by factoring in elements such as operational impact and urgency. Once the combined severity score is set, a \ac{SSVC}-based decision tree incorporates parameters like \emph{Vulnerability Classification}, \emph{Patch Availability}, \emph{System Usage}, and \emph{Asset Criticality}, gathered via a frontend user query to ensure context-informed remediation. 
