...
The current solution integrates data from the \ac{OSV} database into the internal system for vulnerability management. The process ensures efficient synchronization, processing, and storage of vulnerability advisories, aligning them with components defined in software bill of materials (\ac{SBOM}) to identify affected packages and components with vulnerabilities.
...
\begin{itemize}
    ...
    For each vulnerability, the system first queries the \ac{OSV} database to retrieve both the \ac{CVE} identifier and (if available) the \ac{CVSS} vector, leveraging \ac{OSV}'s integration of the entire \ac{GAD} \footnote{\url{https://osv.dev/}}. If no \ac{CVSS} vector is found there, the system sends a request to external sources (e.g., the \ac{NVD}), thereby minimizing the number of lookups. Similarly, the system always retrieves \ac{EPSS} data from the \textit{first.org} database to ensure it remains up to date, as it is not included in the internal \ac{OSV} cache.
    \item \textbf{Caching and Time-Based Retrieval:}  
    The system initiates a daily update cycle by first attempting to retrieve \ac{CVSS} vectors from its local \ac{OSV} database. If unavailable, it sends individual requests to the \ac{NVD} for the missing \ac{CVSS} details, as batch queries for \ac{CVE} IDs are not supported \footnote{\url{https://nvd.nist.gov/developers/vulnerabilities}}. Simultaneously, batch requests are sent to the \textit{first.org} database for \ac{EPSS} scores.
...
The backend checks if a \ac{CVE} is in the cache. If not, the workflow transitions to an \texttt{Error} state, returning a severity score of 10.1 (see section~\ref{subsec:handling-missing-data}), highlighted in pink on the frontend. Users can consult the \emph{Score Explanation} feature (see section~\ref{subsec:ui-explaining-score}) for guidance. If the \ac{CVE} exists, the system checks whether \ac{CVSS} data is cached. If unavailable, an external fetch is performed. The \ac{EPSS} metric is always retrieved externally to ensure it remains up to date, as it is not included in the cache.
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth,height=0.97\textheight,keepaspectratio]{resources/Data_Flow_Vulnerability_Data.png}
...
External data sources such as the \ac{NVD} and the \texttt{first.org} database all impose rate limits to protect against misuse. In this architecture, \ac{CVE} data is fetched from the the local \ac{OSV} database, \ac{CVSS} data from the \ac{NVD}, and \ac{EPSS} data from \texttt{first.org}. For example, \ac{NVD} limits requests to five per 30-second window without an \ac{API} key, and fifty when using one.\footnote{\url{https://nvd.nist.gov/developers/start-here}}
Meanwhile, the the \texttt{first.org} database enforces a threshold of 1.000 requests per hour without a token \footnote{\url{https://api.first.org/\#Rate-Limit}}. Since \ac{CVE} data is retrieved from the local \ac{OSV} database, no rate limits apply.

To cope with these constraints, the backend employs caching and batching to reduce repetitive lookups. It retrieves data only when a record is missing or considered outdated, significantly minimizing external calls and lowering the risk of exceeding rate limits. During nightly updates, requests for \ac{EPSS} scores are processed in batches to further optimize efficiency. As a result, the system operates more reliably handles errors by returning a severity score of 10.1 (see section~\ref{subsec:handling-missing-data}) if fetch errors arise. This status is highlighted in pink on the frontend, where users can consult the score explanation interface (see section~\ref{subsec:ui-explaining-score}) to determine the next steps for resolving the issue.
...
\begin{itemize}
    ...
    \item \textbf{Scalability:} Implement nightly batch updates (when possible) and caching to efficiently manage increasing vulnerability volumes and high \ac{API} traffic.
    ...
    \item \textbf{Consistency:} Apply uniform approaches to caching and scoring so that users have a predictable view of vulnerability metrics, regardless of the used data source.
\end{itemize}
